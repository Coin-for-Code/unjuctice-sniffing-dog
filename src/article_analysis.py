...
CRIME_KEY_WORDS = (
    ...
    'посадовець під підозрою', 'фінансові махінації', 'розтрата бюджетних коштів', 'підозра на корупційне діяння', 'угода', 'службове підроблення',
    'злочинець', 'відмова у перевірці', 'відмивання активів', 'маніпуляції з податками',
    'незаконне управління активами',
    'незаконні витрати', 'фіктивне планування', 'протизаконна власність', 'незаконне управління коштами',
    'неправомірне використання ресурсів', 'державні змови', 'незадекларовані активи', 'перекваліфікація',
...
def identify_criminals_beta(text):
    nlp = spacy.load(NLP_MODEL)
    doc = nlp(text)
    figures = []
    ...
    for ent in doc.ents:
        if ent.label_ == "PER":
            context = [token.lemma_ for token in ent.sent]
            # Подсчет совпадений из контрольного списка
            matches = sum(1 for word in CRIME_KEY_WORDS if word in context)
            # Проверяем, что найдено 3 или более совпадений и нет исключающих ролей
            if matches >= 3 and not any(role in context for role in EXCLUDING_ROLES):
                figures.append(ent.text)

    log.info("Sniffed for these bad boys: %s", str(figures))
    return figures
